...
def test_fixed_time_signal_validation() -> None:
    # Invalid green_time
    with pytest.raises(
        ValueError, match="Green and yellow phase times must be positive"
    ):
...
def test_carryover_timing_overflow() -> None:
    ...
    # ns_green (10s) -> ns_yellow (2s) -> all_red (1s) -> ew_green (remaining
    # 1.0s elapsed)
    ctrl.update(14.0, [])
    assert ctrl.current_phase == "ew_green"
    ...
    # Spans remainder of ew_green (9.0s), ew_yellow (2.0s), all_red (1.0s), which
    # completes cycle 1 (12.0s).
    # Starts cycle 2: ns_green (10.0s), ns_yellow (2.0s), all_red (1.0s),
    # ew_green (10.0s), ew_yellow (2.0s), all_red (1.0s).
